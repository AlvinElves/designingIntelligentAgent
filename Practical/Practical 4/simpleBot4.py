...
def moveIt(canvas, registryActives, registryPassives, count, moves, signalStrengths, dangerThreshold):
    moves += 1
    for rr in registryActives:
        if isinstance(rr, Bot):
        ...
        if moves > numberOfMoves:
            ...
            sys.exit()
    canvas.after(50, moveIt, canvas, registryActives, registryPassives, count, moves, signalStrengths, dangerThreshold)


def trainIt(canvas, window, registryActives, registryPassives, count, moves, signalStrengths, trainingSet):
    moves += 1
    for rr in registryActives:
        if isinstance(rr, Bot):
            look = rr.look(registryActives)
            chargerIntensityL, chargerIntensityR = rr.senseCharger(registryPassives)
            rr.brain(chargerIntensityL, chargerIntensityR)
        if isinstance(rr, Cat):
            rr.brain()
        ...
        if isinstance(rr, Bot):
            registryPassives = rr.collectDirt(canvas, registryPassives, count)
            collision = rr.collision(registryActives)
            trainingSet.append(tuple([look, collision]))
        numberOfMoves = 1000
        if moves > numberOfMoves:
            print("total dirt collected in", numberOfMoves, "moves is", count.dirtCollected)
            canvas.destroy()
    ...
    canvas.after(1, trainIt, canvas, window, registryActives, registryPassives, count, moves, signalStrengths, trainingSet)
...
def train():
    ...
    trainIt(canvas, window, registryActives, registryPassives, count, moves, signalStrengths, trainingSet)
    window.withdraw()
    window.mainloop()
    ...
    for i in range(len(trainingSet)):
        if trainingSet[i][1] is True:
            warningValues.append(trainingSet[i-5][0])

    dangerThreshold = []

    for i in range(0, 9):
        biggest = 0
        for j in range(len(warningValues)):
            if biggest < warningValues[j][i]:
                biggest = warningValues[j][i]

        if biggest != 0:
            dangerThreshold.append(biggest)

    dangerThreshold = sum(dangerThreshold) / len(dangerThreshold)
